  ...
  <section class="dashboard-card">
    <div class="dashboard-card__inner">
      <div class="card-header">
        <div>
          <h3 class="card-title">Top impresoras por uso</h3>
          <p class="card-subtitle">Ranking por serie registrada.</p>
        </div>
      </div>
      ...
      <div v-if="items.length" class="ranking-list">
        <div
          v-for="item in items"
          :key="item.impresora_serie"
          class="ranking-row"
        >
          <div class="ranking-row__meta">
            <span>{{ item.impresora_serie || 'Sin serie' }}</span>
            <strong>{{ formatNumber(item.total_paginas) }} paginas</strong>
          </div>
          <div class="ranking-row__track">
            <div class="ranking-row__bar ranking-row__bar--teal" :style="{ width: `${item.percent}%` }" />
          </div>
        </div>
      </div>

      <div v-else class="empty-state">No hay impresoras para mostrar.</div>
    </div>
  </section>
</template>
